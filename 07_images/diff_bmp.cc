...
bitmap read_BMP(const std::string& filename)
{
    ...
    if (bits_per_pixel != 24) {
        std::cerr << "Panic! : the file " + filename + " has a non-standard amount of bits per pixel" << std::endl;
        f.close();
        return {0,0, nullptr};
    }
...
int main(int argc, char ** argv) {
    if (argc != 4) {
        std::cerr << "Usage: ./diff_bmp <first_file_path> <second_file_path> <result_path>" << std::endl;
        return 2;
    }
    ...
    if (perfect) {
        ...
        if (okay) {
            std::cout << "Every pixel is a neighbour of a pixel with the correct colour\n TEST PASSED" << std::endl;
        }
        else {
